...
**이어하기(자동, 전역)** — 완료 기록이 파일 단위(절대경로+크기+수정시각)로
`data/batch_state/global_index.json`에 기록되어, 중간에 끊겨도 다시 실행하면 완료된
파일은 자동으로 건너뜁니다 (실패한 파일은 재시도, 원본이 변경된 파일은 다시 전사).
기록은 **제출한 폴더와 무관**하게 파일 자체 기준이므로, **상위 폴더 하나만 제출해도
하위 폴더에서 이미 완료한 파일은 건너뛰고 새 파일만 전사**됩니다 — 매달 폴더가
추가되는 구조라면 상위 폴더 한 번 제출이 곧 증분 처리입니다:
```bash
./run.sh batch "/Volumes/studycode/컨설팅 영상_2025" --lang ko
```
특정 폴더만 처음부터 다시 돌리려면:
```bash
./run.sh batch /path/to/folder --fresh
```
웹 UI의 "로컬 폴더 경로" 입력과 Google Drive 폴더 배치에도 동일하게 적용됩니다.

**결과물 자동 정리** — 로컬 폴더 배치의 다운로드 사본은
`~/Downloads/2025년_9월_고1/`처럼 소스 폴더명 기반 폴더에 자동 분류 저장됩니다
